        ...
        this.sqlite.create({
            ...
            this.openDatabase.executeSql('SELECT * FROM wellness WHERE rowid = ?', [this.checkinLogID])
                .then(res => {
                    ...
                    this.date = moment(this.userRecords.date).format();
                    this.dateTime = moment(this.userRecords.date).format();
                    
                    console.log("this.date: " + this.date);
                   // console.log("this.date.toISOString(): " + this.date.toISOString());
                    console.log("this.dateTime: " + this.dateTime);
                    //console.log("this.dateTime.toISOString(): " + this.dateTime.toISOString());
                })
                ...
                },
                {
                    ...
                    handler: () => {
                        if(this.openDatabase != undefined) {
                            ...
                            var fullDate = this.date.substring(0, this.date.indexOf("T")) + "T" + this.dateTime.substring((this.dateTime.indexOf("T")+1));
                            console.log("fullDate: " + fullDate);
                            
                            this.openDatabase.executeSql('UPDATE wellness SET moodScore = ?, dietScore = ?, sleepScore = ?, stressScore = ?, entryNote = ?, date = ? WHERE rowid = ?', [ this.userRecords.moodScore, this.userRecords.dietScore, this.userRecords.sleepScore, this.userRecords.stressScore, this.userRecords.entryNote, fullDate, this.checkinLogID]).then(res => {
